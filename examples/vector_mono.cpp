...
int main(int argc, char **argv) {
  ...
  std::string pathCam0 = pathSeq + "/rgb";
  std::string pathCam1 = pathSeq + "/rgb1";
  ...
  for (int ni = 0; ni < nImages; ni++) {
    ...
    //                    vstrImageLeft[ni]);
    pSLAM->TrackMonocular(imLeft, tframe, std::vector<ORB_SLAM3::IMU::Point>(),
                          vstrImageLeft[ni]);
...
void LoadImages(const std::string &strPathLeft,
  ...
  while (!fTimes.eof()) {
    ...
    if (!s.empty()) {
      ...
      ss << s;
      double t;
      ss >> t;  // start of the exposure time

      vstrImageLeft.push_back(strPathLeft + "/" + std::to_string(t) + ".png");
      vstrImageRight.push_back(strPathRight + "/" + std::to_string(t) + ".png");
      vTimeStamps.push_back(t);

      ss >> t;  // end of the exposure time
    }
  }
  std::cout << "Loaded " << vstrImageLeft.size() << " images." << std::endl;
}
