...
void readColmapCamerasBinary(std::shared_ptr<GaussianMapper> pMapper,
  ...
  if (!file.is_open()) throw std::runtime_error("Invalid file path!");

  const std::size_t num_cameras =
      colmap::ReadBinaryLittleEndian<std::uint64_t>(&file);
  for (std::size_t i = 0; i < num_cameras; ++i) {
    ...
    camera.setModelId(static_cast<Camera::CameraModelType>(
        colmap::ReadBinaryLittleEndian<int>(&file)));
    ...
    for (int l = 0; l < pMapper->num_gaus_pyramid_sub_levels_; ++l) {
      camera.gaus_pyramid_width_[l] =
          camera.width_ * pMapper->kf_gaus_pyramid_factors_[l];
      camera.gaus_pyramid_height_[l] =
          camera.height_ * pMapper->kf_gaus_pyramid_factors_[l];
    }
...
void readColmapImagesBinary(std::shared_ptr<GaussianMapper> pMapper,
                            const std::filesystem::path& path,
                            const std::filesystem::path& images_dir) {
  std::ifstream file(path.string(), std::ios::binary);
  if (!file.is_open()) throw std::runtime_error("Invalid file path!");

  const std::size_t num_reg_images =
      colmap::ReadBinaryLittleEndian<std::uint64_t>(&file);
  for (std::size_t i = 0; i < num_reg_images; ++i) {
    std::uint32_t image_id =
        colmap::ReadBinaryLittleEndian<std::uint32_t>(&file);

    std::shared_ptr<GaussianKeyframe> new_kf =
        std::make_shared<GaussianKeyframe>(image_id);
    ...
    cv::Mat image =
        cv::imread(image_path.string(), cv::ImreadModes::IMREAD_COLOR);
    ...
    for (std::size_t j = 0; j < num_points2D; ++j) {
      ...
      point3D_ids.push_back(
          colmap::ReadBinaryLittleEndian<point3D_id_t>(&file));
    }
    ...
    for (point2D_idx_t point2D_idx = 0; point2D_idx < num_points2D;
         ++point2D_idx) {
      if (point3D_ids[point2D_idx] !=
          std::numeric_limits<point3D_id_t>::max()) {
...
void readColmapPoints3DBinary(std::shared_ptr<GaussianScene> scene,
                              const std::filesystem::path& path) {
  std::ifstream file(path.string(), std::ios::binary);
  if (!file.is_open()) throw std::runtime_error("Invalid file path!");

  const std::size_t num_points3D =
      colmap::ReadBinaryLittleEndian<std::uint64_t>(&file);
  for (std::size_t i = 0; i < num_points3D; ++i) {
    ...
    const std::size_t track_length =
        colmap::ReadBinaryLittleEndian<std::uint64_t>(&file);
    for (std::size_t j = 0; j < track_length; ++j) {
      const std::uint32_t image_id =
          colmap::ReadBinaryLittleEndian<std::uint32_t>(&file);
      const point2D_idx_t point2D_idx =
          colmap::ReadBinaryLittleEndian<point2D_idx_t>(&file);
    }

    std::cout << "Loading colmap point3D " << point3D_id << ", " << (i + 1)
              << "/" << num_points3D << "\r";
    std::cout.flush();
...
void readColmapScene(std::shared_ptr<GaussianMapper> pMapper) {
  ...
  std::filesystem::path cameras_intrinsic_file =
      model_params.source_path_ / "sparse/0" / "cameras.bin";
  std::filesystem::path cameras_extrinsic_file =
      model_params.source_path_ / "sparse/0" / "images.bin";
  std::filesystem::path points3D_bin_file =
      model_params.source_path_ / "sparse/0" / "points3D.bin";
...
int main(int argc, char** argv) {
  ...
  std::shared_ptr<GaussianMapper> pGausMapper =
      std::make_shared<GaussianMapper>(
          ...
          selector_enabled_override, *selector_config_path);
